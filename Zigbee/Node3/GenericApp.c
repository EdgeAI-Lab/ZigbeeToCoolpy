...
uint16 GenericApp_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    while ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event )
      {
        ...
        case ZDO_STATE_CHANGE:
          ...
          {
            //Send short addr to coordinator
            osal_set_event(GenericApp_TaskID,SEND_ADDR_EVT);
  ...
  if ( events & SEND_ADDR_EVT )
  {
    // Send "the" message
    My_SendTheShortAddr();

    // return unprocessed events
    return (events ^ SEND_ADDR_EVT);
  }
